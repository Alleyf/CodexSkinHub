...
**Q: doctor 报 `MISS engine node`（v0.3.0 及更早），或者我的 Node 装在非系统盘 / nvm 管理的目录？**
旧版用 `NODE_EXE !== process.execPath` 判断引擎 node 是否存在——当 shim 恰好用引擎自带的 node 启动 CLI 时两者天然相等，被误判为「没找到」（v0.3.1 已修复该判断）。v0.3.2 起逻辑收敛为一句话：优先用引擎自带运行时（`engine\runtime\node\`），否则直接用**当前正在运行 CLI 的那个 node**——它无论如何都是一个能用的 node，不需要任何额外探测。Node 装在任意盘符 / nvm / volta 管理的目录都不影响；开机自启脚本在两者皆缺失时静默退出，登录时不会弹窗。`doctor` 会显示实际采用的 Node 路径。
